...
- (void)pinchAction
{
    ...
    if (self.pinchToShow.state == UIGestureRecognizerStateBegan) {
        ...
        switch (self.actionNo) {
            case TVPinchRoot:
                [TVRootViewCtlBox sharedBox].transitionPointInRoot = [self pointBy:self.pinchToShow inView:[[UIApplication sharedApplication] keyWindow].rootViewController.view];
                break;
            case TVPinchToSave:
                [TVRootViewCtlBox sharedBox].transitionPointInRoot = [self pointBy:self.pinchToShow inView:self.parentViewController.view];
            default:
                break;
        }
